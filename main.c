...
static struct xconfig_option evilwm_options[] = {
	...
	{XCONFIG_CALL_1, "annotate.sweep.outline",
			&set_annotate_sweep_outline},
...
main(int argc, char *argv[])
{
...

	{
		...
		if (home) {
			char       *conffile =
				xmalloc(strlen(home) + sizeof(CONFIG_FILE) +
				2);
...
setup_display(void)
{
	...
	if (!font) {
		LOG_ERROR
			("couldn't find a font to use: try starting with -fn fontname\n");
		exit(1);
	}
	...
	for (unsigned i = 0; i < 8; i++) {
		for (unsigned j = 0; j < (unsigned int) modmap->max_keypermod;
			j++) {
			if (modmap->modifiermap[i * modmap->max_keypermod +
					j] == XKeysymToKeycode(dpy,
					XK_Num_Lock)) {
				numlockmask = (1 << i);
...
#ifdef RANDR
	{
		...
		have_randr =
			XRRQueryExtension(dpy, &randr_event_base, &e_dummy);
		if (!have_randr) {
			LOG_DEBUG
				("XRandR is not supported on this display.\n");
		}
	}
#endif
	/* Xinerama extension? */
#ifdef XINERAMA
	{
		...
		have_xinerama =
			XineramaQueryExtension(dpy, &xinerama_event, &e_dummy)
...
setup_screens(void)
{
	...
	for (int i = 0; i < num_screens; i++) {
		...
		unsigned long *vdesks =
			get_property(screens[i].root,
			xa_evilwm_current_desktops, XA_CARDINAL, &vdesks_num);

		for (int j = 0; j < screens[i].num_physical; j++) {
			if (vdesks && vdesks_num > (unsigned) j)
				screens[i].physical[j].vdesk = vdesks[j];
			else
				screens[i].physical[j].vdesk =
					KEY_TO_VDESK(XK_1) + MIN(opt_vdesks,
					(unsigned) j);
		}
...
#ifdef RANDR
		if (have_randr) {
			XRRSelectInput(dpy, screens[i].root,
		...
		XAllocNamedColor(dpy, DefaultColormap(dpy, i), opt_fg,
			&screens[i].fg, &dummy);
		XAllocNamedColor(dpy, DefaultColormap(dpy, i), opt_bg,
			&screens[i].bg, &dummy);
		XAllocNamedColor(dpy, DefaultColormap(dpy, i), opt_fc,
			&screens[i].fc, &dummy);

		screens[i].invert_gc =
			XCreateGC(dpy, screens[i].root,
			GCFunction | GCSubwindowMode | GCLineWidth | GCFont,
			&gv);

		XChangeWindowAttributes(dpy, screens[i].root, CWEventMask,
			&attr);
